...
build:
	...
	docker cp ./.docker/composer/composer.phar magento-fpm:/composer.phar
...
magento-composer:
	docker exec -it magento-fpm sh -c 'php -d memory=-1 ../composer.phar $(filter-out $@,$(MAKECMDGOALS))'
...
magento-download:
	docker exec -it magento-fpm sh -c 'php -d memory=-1 ../composer.phar create-project --repository-url=https://repo.magento.com/ magento/project-community-edition .'
